...
public class StatisticsHandler {
	...
	public StatisticHandlerArraylist(Manager manager){
		this. manager = manager;
	}
	
	public double[] typeStats() {
		int typeCode[];     
		String typeName[];
		double results[];
		int eventCounter = 0;
		ArrayList <Event> list = manager.getEvent();
		
		typeCode = list.get(0).getTypeCodeArray();							//quanto è grande questo vettore? se poi aggiungo un tipo di 
		...
		for (int j=0; j<Array.getLength(typeCode); j++) {
			for (int i=0; i<Array.getLength(list); i++) {
				...
				if(list.get(i).getTypeCode() == typeCode[j]) {
					-----
